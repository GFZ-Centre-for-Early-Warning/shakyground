...
class PlanarSurfaceAlt(PlanarSurface):
    ...
    def get_ry0_distance(self, mesh):
        ...
        dst1 = geodetic.distance_to_arc(
            self.top_left.longitude,
            self.top_left.latitude,
            (self.strike + 90.0) % 360,
            mesh.lons,
            mesh.lats,
        )

        dst2 = geodetic.distance_to_arc(
            self.top_right.longitude,
...
class FiniteRuptureSampler(object):
    ...
    def get_finite_rupture_distances(
        self,
        nsamples,
        centroid,
        magnitude,
    ...
    def sample_ruptures(
        self, centroid, magnitude, nsamples, mechanisms="U", dimensions=None
    ):
        """
    ...
    def sample_mechanisms(self, nsamples, mechanisms):
        ...
        if isinstance(mechanisms, PMF):
            ...
            strikes = np.random.uniform(0.0, 360.0, nsamples)
    ...
    def sample_hypocentres(self, nsamples, sofs):
        ...
        for key in sof_counter:
            ...
            all_valid = np.logical_and(
                np.logical_and(pos[:, 0] >= 0.0, pos[:, 0] <= 1.0),
                np.logical_and(pos[:, 1] >= 0.0, pos[:, 1] <= 1.0),
            )
    ...
    def build_ruptures(self, centroid, strikes, dips, lengths, widths, hypo_locs):
        ...
        """
        ruptures = []
        rstrikes = strikes * np.pi / 180.0
        rdips = dips * np.pi / 180.0
    ...
    def get_best_matching_rupture(
        self, rjb, rrup, ruptures, distances, hypocentral_distances, rdim=0.0
    ):
        """
